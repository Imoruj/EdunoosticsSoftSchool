...
export async function GET(req: NextRequest) {
    try {
        ...
        if (isStudent) {
            // Students see only APPROVED SOWs for subjects they're enrolled in (current session)
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const { subjectId, classArmId, sessionId, title } = body;

        if (!subjectId || !classArmId || !sessionId) {
            return NextResponse.json({ error: "subjectId, classArmId, and sessionId are required" }, { status: 400 });
        }

        // Teachers must have a TeacherSubject assignment for this subject+classArm
        if (!isAdmin) {
            const assignment = await prisma.teacherSubject.findFirst({
                where: { teacherId: user.id, subjectId, classArmId },
            });
            if (!assignment) {
        ...
        // Validate subject and classArm belong to school
        const [subject, classArm] = await Promise.all([
            prisma.subject.findFirst({ where: { id: subjectId, schoolId } }),
            prisma.classArm.findFirst({ where: { id: classArmId, class: { schoolId } } }),
        ]);
        if (!subject || !classArm) {
            return NextResponse.json({ error: "Invalid subject or class arm" }, { status: 400 });
        }

        // Auto-generate title if not provided
        const resolvedTitle = title?.trim() ||
            `${subject.name} — ${classArm.armName} — ${academicSession.name}`;

        // Create SOW + SchemeOfWorkTerm records for each term in the session
        const sow = await prisma.$transaction(async (tx) => {
            const created = await tx.schemeOfWork.create({
                data: {
                    schoolId,
                    subjectId,
                    classArmId,
            ...
            // Create a SchemeOfWorkTerm for each term in the session
            if (academicSession.terms.length > 0) {
                await tx.schemeOfWorkTerm.createMany({
                    data: academicSession.terms.map((term) => ({
            ...
            return tx.schemeOfWork.findUniqueOrThrow({
                where: { id: created.id },
                include: {
